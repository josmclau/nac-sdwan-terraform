...
Verify Policy Objects SLA Class List {{ sla.name }}
   ...
   IF    ${app_probe_id} == []
      Should Be Equal Value Json String   ${sla.json()}   $..appProbeClass   {{ sla.app_probe_class | default("not_defined") }}  msg={{ sla.name }}: App Probe Class
   ELSE
      ${app_probe_object}=   GET On Session   sdwan_manager   /dataservice/template/policy/list/appprobe/${app_probe_id[0]}
      Should Be Equal Value Json String   ${app_probe_object.json()}   $..name   {{ sla.app_probe_class | default("not_defined") }}  msg={{ sla.name }}: App Probe Class
   END
